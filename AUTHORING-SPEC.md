...
- **A10 (extends the verb set — the gap mark).** `VerbName` gains `gap`, taking a curve id, calling `FrameState.markGap`. Ratified by the owner's approval of the Class-A road plan, on the same basis as A1–A9. The statute already required the mark and could not set it: clause 24 has panels either side of a gap referencing the same authored GAP curve, A2 rules that a shutline does not break flow unless coincident with a character line, `markGap` and `QuiltSpec.gaps` were both present — and the ratified verb list had `crease` and nothing else, so no curve in any document could be a gap. Consequences: groove engraving reads `quilt.gaps`, not `quilt.creases`; a creased character line is no longer engraved as though it were a shutline; the tangent field continues to correct across a gap (A2) and to leave creases alone. Additive to the verb set, so no stored document re-hashes.
- **A11 (extends the verb set — corner fairing).** `VerbName` gains `fair-corners`, taking a maximum break angle, rotating each adjacent curve's END tangent at every vertex the network turns badly until the two patches there share one tangent plane. Ratified by the owner's approval of the Class-A road plan, on the same basis as A1–A10. It moves the model, which is why it is a verb and not a derived field: clause 20 says a cell evaluates from ITS four boundary curves, so a fairing computed at evaluation time would put the surface somewhere other than through the curve the designer drew. Bounded three ways: it brings the crossing curves COPLANAR and not tangent, which is a fraction of the swing and is what the surfacing actually needs; it leaves anything sharper than the break angle exactly as authored, the same judgment the render's crease angle makes; and it moves one control point per curve end, never an endpoint, so every weld holds and the quilt stays watertight by the mechanism it always used. Fixed at two passes — moving a control point moves interior trim endpoints and so moves other cells' corners, and iterating to convergence would make the result depend on a tolerance rather than on the state.
- **A12 (extends the verb set — fullness).** `VerbName` gains `fullness`, taking a set of cells and a positive multiplier. It is the first control this frame has ever had over a patch INTERIOR: clause 20 says a cell evaluates from its four boundary curves, and everything between them follows, which is exactly why the audit's fullness row has read "interiors are determined, never designed" from the beginning and why a body authored this way reads as slabs between its feature lines. The amount scales the TRANSVERSE part of the cross-boundary derivative and nothing else. Ratified by the owner's instruction of 23 August — *"my brain fills the space in between the enclosed lines with an unbroken smooth panel… like a real die stamped panel"* — on the same basis as A1–A11. It is safe against every guarantee the frame makes, and by argument rather than by tolerance: it cannot move a boundary point, because the boundary IS the curve and clause 20 is untouched; and it cannot rotate a tangent plane, because span{C′, λd̂} does not depend on λ. G0 stays bit-identical and G1 stays exact at any amount, which is checked at both ends of the seam. What it does change is the shape, by tens of millimetres, which is why it is a verb the owner sets and not a default the build chooses.
