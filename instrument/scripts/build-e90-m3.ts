/**
 * Build the BMW E90 M3 LCI — the fifth car, and the first SEDAN.
 *
 * Four doors, two seat rows, a boot: every body in this repository so far has
 * had one row and at most one door per side, and the grammar has never been
 * asked for a B-pillar, a second shut ring through the flanks, or a deck that
 * is neither a clamshell nor a tailgate. This car asks for all three.
 *
 * WHAT THAT TESTS. The occupant array's ROWS — two of them, five people —
 * drive a roofline that has to stay up for a fifth of the car's length; the
 * classic front-mid package (the whole V8 behind the front axle, a propshaft
 * down a real tunnel, the tank under the rear bench) drives a long bonnet the
 * way the E-Type's six did, on a body whose proportions are otherwise
 * nothing like it; and the M3's flares put the widest point of the whole car
 * over the REAR arch, which no car in here has had.
 *
 * Body datum: X = 0 at the NOSE, Y across from the centreline, Z up from the
 * ground plane. Millimetres.
 *
 *   npx tsx scripts/build-e90-m3.ts
 */
...
import {
  e90M3Config, E90_FRONT_DIAMETER, E90_FRONT_OVERHANG, E90_FRONT_TIRE_WIDTH, E90_FRONT_TRACK,
  E90_PROFILE, E90_PROFILE_TOLERANCE_MM, E90_REAR_DIAMETER, E90_REAR_OVERHANG,
  E90_REAR_TIRE_WIDTH, E90_REAR_TRACK, E90_HEIGHT, E90_LENGTH, E90_WHEELBASE, E90_WIDTH,
} from "@car/fixtures";
...
const personInBody = (): CabinPerson => {
  ...
  const at = (name: RegExp): Pt3 => {
    ...
    return [o[0] + pose.origin[0] + E90_FRONT_OVERHANG, DRIVER_Y, o[2] + pose.origin[2]];
  };
  return {
...
/**
 * Back to 370, after the one car whose driver sat on the centreline. A
 * left-hand-drive sedan with a real transmission tunnel: the driver sits
 * beside it, and every cabin reading below is taken at that shoulder.
 */
const DRIVER_Y = 370;

const NOSE = E90_FRONT_OVERHANG;
const FRONT_AXLE_X = NOSE;
const REAR_AXLE_X = NOSE + E90_WHEELBASE;
/**
 * TWO WHEEL DIAMETERS AGAIN — the F1 built the per-axle machinery and the E90
 * inherits it for the opposite reason: a staggered fitment. 653.2 mm at the
 * front on a 245/40 and 669.2 at the rear on a 265/40, both on 18s, and the
 * REAR pair is the wider and the taller — which with the M3's flares is why
 * the widest point of this whole car is over the rear axle.
 *
 * TWENTY-EIGHT MILLIMETRES OF RADIAL CLEARANCE. The bonnet gives more room
 * than the F1's 685 mm nose did — the front crown lands at 681 under a
 * beltline drawn at 740 — but the same lesson bounds it: an arch LIP is a
 * panel edge, not a clearance envelope, and the tyre jounces up into the
 * wheelhouse behind it.
 */
const FRONT_WHEEL_R = E90_FRONT_DIAMETER / 2;
const REAR_WHEEL_R = E90_REAR_DIAMETER / 2;
const ARCH_CLEAR = 28;
const FRONT_ARCH_R = FRONT_WHEEL_R + ARCH_CLEAR;
const REAR_ARCH_R = REAR_WHEEL_R + ARCH_CLEAR;
/**
 * How far the lip stands proud of the tyre's outer face. SIX, and this time
 * the REAR axle is why: a 265 section on a 1539 track puts the tyre's outer
 * face at 902 mm against a published half-width of 908.5. Six and a half
 * millimetres of daylight, and the lip takes six of it — which makes the rear
 * arch lip the widest thing on the car, at 908, exactly as the flares on the
 * real one are. The M3's pumped arches are not decoration here; they are the
 * only way the tyres fit.
 */
const ARCH_LIFT = 6;
/**
...
const placedParts = (): CarriedPart[] => {
  ...
  for (const part of car.input.parts) {
    ...
    const c: Pt3 = [
      pose.origin[0] + o[0] + E90_FRONT_OVERHANG,
      pose.origin[1] + o[1],
      pose.origin[2] + o[2],
    ];
...
// ── 2. author the body ────────────────────────────────────────────────────
const s = createSession("BMW E90 M3 LCI");
const side = { kind: "side" as const };

const LEN = E90_LENGTH, HW = E90_WIDTH / 2, FLOOR = 120, TOP = E90_HEIGHT;
...
// A sedan's character is carried by three nearly-level lines and the spaces
// between them: a beltline that runs the whole car at half its height, a
// sill that runs parallel under it, and a roofline that rises once, holds
// for a fifth of the length, and falls once. Nothing swoops. The E90 M3 adds
// exactly two events to that: a flare over each axle — and the rear one is
// the wider, which no car in this repository has had.
...
// Four stations at x = 0, 1527, 3053 and 4580. On a sedan the shoulder and
// the rocker run nearly parallel in plan — the body side is one wall — and
// the rocker only beats the shoulder over the arches, where the flares live.
const SHOULDER_Y = [720, 862, 866, 780] as const;
const ROCKER_Y = [760, 866, 870, 740] as const;
...
// The beltline climbs gently the whole way — wing edge to door top to deck
// shoulder — and the deck holds it: a modern sedan's boot is nearly as high
...
const railY0 = track(RAIL_Y0 * 0.71, RAIL_Y0 * 0.99, RAIL_Y0 * 0.87, RAIL_Y0 * 0.76);
const railZ0 = track(640, 900, 1420, 950);
...
const FRONT_LIP = E90_FRONT_TRACK / 2 + E90_FRONT_TIRE_WIDTH / 2 + ARCH_LIFT;
const REAR_LIP = E90_REAR_TRACK / 2 + E90_REAR_TIRE_WIDTH / 2 + ARCH_LIFT;
...
const rockerPlanY = profile([
  ...
  // that are NOT span ends are inside the arch spans — 700, 1150, 3500,
  // 3900 — and those are read by the arcs' own four-point fit, which is what
 ...
 * The beltline's plan.
 *
 * A GENTLE WAIST, not the F1's 40 mm tuck: the E90's doors pull in six
 * millimetres between the flares — the scallop a side view shows as shadow —
 * and then the HIPS swell past everything, 902 at the rear axle against 880
 * at the front. Widest at the back: the one plan fact that makes an M3 an M3.
 */
 ...
 *
 * PLAN pulls in through the greenhouse — 508 at the cowl to 436 over the
 * B-pillar, which is the tumblehome of a sedan whose glass leans in — and
 * eases back OUT over the deck to the boot's shoulders. Ahead of the cowl
 * the same line is the bonnet's side edge above the wing.
 *
 * HEIGHT is the sedan roofline in one table: a bonnet that climbs steadily,
 * a screen at 38 degrees, a roof that holds within ten millimetres of its
 * peak from the header to mid-cabin, a backlight at 36, and a deck that
 * finishes near the beltline. One rise, one hold, one fall.
 */
const railPlanY = profile([
  [0, 402], [280, 458], [fA, 482], [FRONT_AXLE_X, 500], [fB, 508],
  [1620, 505], [1720, 498], [1975, 472], [2330, 446], [2610, 436],
  [3000, 440], [3230, 450], [rA, 455], [3640, 470], [REAR_AXLE_X, 472],
  [rB, 464], [4270, 448], [LEN, 380],
]);
const railZdrawn = profile([
  [0, 640], [280, 706], [fA, 728], [FRONT_AXLE_X, 768], [fB, 808],
  // The cowl, then the screen: 480 mm of rise in 610 of length. The same
  // event as every closed car; on this one it starts at 0.35 of the length.
  [1620, 878], [1720, 930], [1975, 1155], [2330, 1410],
  // The hold: 30 mm of fall in 670 mm of roof. Four heads under it.
  [2610, 1440], [3000, 1398],
  // The backlight and the deck: the fall is all done by 3640, and the boot
  // carries the line out nearly level, finishing 300 mm above the sill.
  [3230, 1296], [rA, 1210], [3640, 1004], [REAR_AXLE_X, 998],
  [rB, 976], [4270, 962], [LEN, 938],
]);
...
const shoulderZprofile = profile([
  // OVER EACH ARCH IT IS PINNED ABOVE THE CROWN — 740 at the front axle over
  // a crown at 681, 915 at the rear over 697 — and between them it does the
  // one thing a sedan beltline does: rises gently, the whole way, wing edge
  // to door top to deck shoulder, and never comes back down until the last
  // 300 mm of boot.
  [0, 655], [280, 690], [fA, 715], [FRONT_AXLE_X, 740], [fB, 768],
  [1620, 810], [1975, 840], [2610, 872],
  [rA, 900], [REAR_AXLE_X, 915], [rB, 908], [4270, 898], [LEN, 885],
]);
 ...
 */
const SHOULDER_X = [
  0, 280, fA, FRONT_AXLE_X, fB,
  1620, 1975, 2610,
  rA, REAR_AXLE_X, rB, 4270, LEN,
];

const SPAN_X = [
  0, 280, fA, FRONT_AXLE_X, fB,
  1620, 1720, 1975, 2330, 2610, 3000, 3230,
  rA, 3640, REAR_AXLE_X, rB, 4270, LEN,
];
...
const crownRise = profile([
  // POSITIVE THE WHOLE LENGTH, and largest over the engine: this is the
  // POWER DOME. The E-Type and the F1 both carry a VALLEY down the nose,
  // because their wings crown above the panel between them; the E90's bonnet
  // does the opposite — the centre stands ~18 mm proud of its own side
  // edges, because the S65's intake plenum is under it. The one crown table
  // in this repository whose biggest number is over an engine.
  [0, 4], [280, 8], [fA, 10], [FRONT_AXLE_X, 14], [fB, 18],
  [1620, 12], [1720, 10], [1975, 8], [2330, 6], [2610, 5], [3000, 6],
  [3230, 8], [rA, 8], [3640, 6], [REAR_AXLE_X, 6], [rB, 5], [4270, 4], [LEN, 3],
]);
const crownZ = (x: number): number => railZ(x) + crownRise(x);

/** How low the car may sit, from the brief. Nothing below this is packageable. */
const GROUND_CLEARANCE = e90M3Config.brief.groundClearanceMm.value;
...
const STATIONS: {
  ...
  // the prow bulge is small — a moulding wrapped round a wide face.
  { x: 90,   floor: 250, hip: 800, hipAt: 0.45, sailBulge: 12, name: "nose-tip" },
  { x: 280,  floor: 130, hip: 860, hipAt: 0.45, sailBulge: 16, name: "nose-mouth" },
  { x: archMouth(FRONT_AXLE_X, FRONT_ARCH_HALF)[0], floor: 125, hip: 886, hipAt: 0.32, sailBulge: 10, name: "arch-front-lead" },
...
// ── panel gaps ────────────────────────────────────────────────────────────
// A panel is a connected run of cells with the GAP-marked curves cut, and a
// SEDAN is the most-panelled thing this tool has drawn: bonnet, four wings
// and doors, a boot. Where the F1 had two rings and three pressings, this car
// has four shut stations and they are not all the same KIND of shut:
//
...
//   flare begins, which is where BMW put it.
//
...
const NOSE_SHUT = "cowl";          // x = 1620 — bonnet shut + wing/door gap
const DOOR_SHUT = "b-pillar";      // x = 2610 — door-to-door, flanks and sails
const REAR_DOOR_SHUT = "arch-rear-lead";   // rear door's trailing edge
const TAIL_SHUT = "decklid-lead";  // x = 3640 — the boot's leading edge
...
// Creased as well as gapped, because a shut line IS a tangent break — the
// panel either side of it is a separate pressing.
...
for (const name of [DOOR_SHUT, REAR_DOOR_SHUT]) {
  const sec = stationOf(name);
  for (const id of [...sec.sails, ...sec.flanks]) {
    s.apply("crease", { curveId: id });
    s.apply("gap", { curveId: id });
  }
}
...
// body side outer meets the floor pan, and this car IS a unibody — the first
// in the repository whose sill gap is exactly what it looks like.
for (const rocker of rockerIds) {
  for (const id of rockerSpans.get(rocker)!) s.apply("gap", { curveId: id });
}

// THE RAIL, CUT IN THREE, AND EACH PIECE IS A DIFFERENT THING — which is the
// strongest case yet for the third master line being real. Ahead of the cowl
// it is the BONNET'S SIDE EDGE above the wing: creased and softened, 20 mm
// opening to 60 by the nose, which is the soft shoulder every E9x bonnet
// carries into its kidney line. Through the greenhouse it is the ROOF
// CHANNEL — the seam the door frames seal against — creased and gapped, the
// E-Type's drip-rail answer on a car that genuinely has one. Aft of the boot
// shut it is the BOOTLID'S SIDE EDGE: creased and gapped, because the lid is
// its own pressing and its gap runs along the deck shoulder, which is why the
// boot ring above only crossed the deck.
for (const rail of railIds) {
  ...
  s.apply("soften", { curveId: fwd, radius: 20, endRadius: 60 });
  s.apply("crease", { curveId: channel });
  s.apply("gap", { curveId: channel });
  // The C-pillar's edge, backlight top to boot: a fold the sail and deck
  // bands genuinely meet at, softened to the wide roll the real quarter has.
  s.apply("crease", { curveId: cEdge });
  s.apply("soften", { curveId: cEdge, radius: 24 });
  s.apply("crease", { curveId: boot });
  s.apply("gap", { curveId: boot });
}

// THE SWAGE LINE — the beltline wearing amendment A12 the way the real car
// wears its character line. On the E90 it rises from the front arch blister,
// runs just under the door handles, and FADES as it approaches the tail
// lamp. So: crisp over the front wing (R10), a shade softer and constant
// through the doors (R8 to R18), and opening right out over the quarter
// (R18 to R60) — a line that dies into the deck exactly where the LCI lamp
// wraps onto the boot. Creased throughout, because the crease says the two
// bands MEAN to disagree and the radius says by how much rounds.
for (const shoulder of shoulderIds) {
  const [wing, rest] = cutSpanAt(shoulder, stationX(NOSE_SHUT));
  const [doors, quarter] = cutSpanAt(rest, stationX(REAR_DOOR_SHUT));
  s.apply("soften", { curveId: wing, radius: 10 });
  s.apply("soften", { curveId: doors, radius: 8, endRadius: 18 });
  s.apply("soften", { curveId: quarter, radius: 14, endRadius: 36 });
}
...
const F_HALF = E90_FRONT_TIRE_WIDTH / 2;
const R_HALF = E90_REAR_TIRE_WIDTH / 2;
if (process.env["NOWHEELS"] !== "1") {
  wheel(FRONT_AXLE_X, FRONT_WHEEL_R, F_HALF, E90_FRONT_TRACK / 2 - F_HALF);
  wheel(REAR_AXLE_X, REAR_WHEEL_R, R_HALF, E90_REAR_TRACK / 2 - R_HALF);
}
...
// ── the chassis: a UNIBODY, drawn as the members that matter ─────────────
// An E90 is a steel unibody: the floor pan, sills, pillars and rails are one
// welded shell, with an aluminium front subframe under the engine and a
// steel cradle under the rear multilink. `makeSubstrate` still knows one
// construction style, so the config declares body-on-frame and the "rails"
// it describes are the floorpan's own pressed longerons — the same
// accommodation the F1's tub made, third car running, and recorded as such.
//
...
if (process.env["NOCHASSIS"] !== "1") {
  ...
  // Floor longerons, under the cabin, bulkhead to bulkhead.
  beam("longeron", {
    view: side,
    a: [BULK, RAIL_Z - RAIL_H / 2], b: [SEAT_BULK, RAIL_Z + RAIL_H / 2],
    depth: RAIL_W, at: RAIL_Y - RAIL_W / 2,
  }, true);
  // The sills, under the four door apertures. Pressed steel, 160 deep — a
  // road sedan's pinch-weld sill, not the F1's 230 mm climbing wall.
  ...
  // them.
  const KICK_Z = 360;
  strut("rear-rail-kick", [SEAT_BULK, RAIL_Y, RAIL_Z], [REAR_AXLE_X, RAIL_Y - 30, KICK_Z], 48, 64, true);
  ...
  for (const [nm, cx] of [["cradle-front", REAR_AXLE_X - 150], ["cradle-rear", REAR_AXLE_X + 170]] as const) {
    beam(nm, {
      view: { kind: "front" as const },
      a: [-460, 190], b: [460, 250], depth: 60, at: cx,
    });
  }

  // ── the suspension ──────────────────────────────────────────────────────
  const F_HUB_Y = E90_FRONT_TRACK / 2 - 62;
  const R_HUB_Y = E90_REAR_TRACK / 2 - 62;
  // FRONT: a strut — the lower wishbone on the subframe, the "upper link"
  // the strut itself, springing on the tower top.
  suspensionCorner(kit, {
    tag: "FL", axleX: FRONT_AXLE_X, hubY: F_HUB_Y, axleZ: FRONT_AXLE_Z,
    lowerIn: [FRONT_AXLE_X, BAY_Y - 60, 190], upperIn: [FRONT_AXLE_X, 540, TOWER_TOP - 20],
    springTop: [FRONT_AXLE_X, 555, TOWER_TOP],
  });
  ...
  suspensionCorner(kit, {
    ...
    lowerIn: [REAR_AXLE_X, 420, 220], upperIn: [REAR_AXLE_X, 450, 430],
    springTop: [REAR_AXLE_X - 40, 470, 540],
  });
  ...
  // Every point below is READ off the master lines. A sedan finally uses the
  // whole alphabet: A-pillar at the screen, a REAL B-pillar between the
  ...
  // ── crash structure ─────────────────────────────────────────────────────
  // THE FIRST CAR WITH STROKE TO SPARE. The F1 had 63% of the front stroke
  // its substrate asked for, because a mid-engined car has no nose to crush;
  // this one has a metre of bay ahead of the block and a whole boot behind
  // the tank, so both beams sit where the skin is and the strokes come out
  ...
  for (const [nm, bx, z, half] of [
    ["bumper-front", noseBeamX, 420, 620], ["bumper-rear", tailBeamX, 560, 640],
  ] as const) {
    beam(nm, {
      view: { kind: "front" as const },
...
const MATERIALS = {
  paint: CATALOGUE["Interlagos Blue"]!,
  screen: CATALOGUE["windscreen"]!,
  backlight: CATALOGUE["backlight"]!,
  sideGlass: CATALOGUE["side glass"]!,
  // Back to painted steel: a unibody is the one construction the chassis
  // finish was actually drawn for.
  chassis: CATALOGUE["chassis"]!,
  under: CATALOGUE["undertray"]!,
  // TWO tyre names, because the fitment is staggered and the two axles
  // genuinely wear different tyres — the F1's per-axle wheels, one step on.
  tyreFront: CATALOGUE["245/40R18"]!,
  tyreRear: CATALOGUE["265/40R18"]!,
  rim: CATALOGUE["alloy"]!,
} as const;
...
writeFileSync(new URL("../cars/e90-m3.car.json", import.meta.url), JSON.stringify(doc));
writeFileSync(new URL("../../e90-m3.stl", import.meta.url), writeStlBinary({ ...printed, normals: shaded.normals }, "e90-m3"));
...
console.log("\nBMW E90 M3 LCI — the fifth car, and the first sedan\n");
...
line("  published 2008", `${E90_LENGTH} × ${E90_WIDTH} × ${E90_HEIGHT} mm (SOURCED — see e90-m3.ts)`);
...
line("head under the roof", !cabin.roofed
  ...
  : cabin.headroom! >= 0
    ...
    : `NONE — the head is ${(-cabin.headroom!).toFixed(0)} mm THROUGH the roof, which on a 1447 mm sedan with two seat rows would be a real fault rather than a supercar's honest squeeze`);
...
{
  const th = cabin.sections.filter((sc) => sc.x > 1800 && sc.x < 3200 && sc.width > 1200);
...
// hundred. `scripts/body-profile.ts` prints the whole table.
{
  ...
  for (const st of E90_PROFILE) {
    ...
    if (Math.abs(dw) > E90_PROFILE_TOLERANCE_MM || Math.abs(dz) > E90_PROFILE_TOLERANCE_MM) over++;
    if (process.env["DBG"] === "1") {
  ...
  const corners = [
    { name: "wheel-FL", at: [FRONT_AXLE_X, E90_FRONT_TRACK / 2, FRONT_AXLE_Z] as Pt3 },
    { name: "wheel-FR", at: [FRONT_AXLE_X, -E90_FRONT_TRACK / 2, FRONT_AXLE_Z] as Pt3 },
    { name: "wheel-RL", at: [REAR_AXLE_X, E90_REAR_TRACK / 2, REAR_AXLE_Z] as Pt3 },
    { name: "wheel-RR", at: [REAR_AXLE_X, -E90_REAR_TRACK / 2, REAR_AXLE_Z] as Pt3 },
  ];
  ...
  // whose shape is package-driven should be able to say WHERE, and which part.
  {
    ...
    if (obeyed.length === 0 && worstLift < 0.5) {
      ...
      line("package vs styling", `${obeyed.length} station bound${obeyed.length === 1 ? "" : "s"} moved by the hard package · ` +
        `roofline lifted ${worstLift.toFixed(0)} mm · on a front-engined sedan the package bites at the ` +
  ...
  if (crushFit) {
    ...
    line("crush stroke", `front ${pct(crushFit.frontAsked, crushFit.frontGot)} · ` +
      `rear ${pct(crushFit.rearAsked, crushFit.rearGot)} — and this car is the first with room to spare at both ends`);
  }
  ...
  line("profile vs the real car",
    ...
    `${over} of ${E90_PROFILE.length} stations outside ${E90_PROFILE_TOLERANCE_MM} mm (envelope SOURCED, stations ASSUMED)`);
}
line("triangles", `${(mesh.indices.length / 3).toLocaleString("en-GB")}`);
console.log("\nwrote cars/e90-m3.car.json and the STL beside it\n");
